...
int main( int nArgc, char ** papszArgv )
{
		...
		while((roadFeature=roadLayer[i]-> GetNextFeature())!= NULL)
		{
			...
			{	
				...
				for(int i=1;i<pointCount-1;i++)
				{
					...
					{
						double tmpd=distanceP2P(edgexList[edgeLen][len-1],edgeyList[edgeLen][len-1],px,py);
						if(tmpd>resolution)
						{
							edgexList[edgeLen][len]=px;
							edgeyList[edgeLen][len]=py;
							edgelengthList[edgeLen][len-1]=tmpd;
	...
	double* facilitypxSimplify = (double*)malloc(facilityCount*MAX_POINT_NUM*sizeof(double));
	double* facilitypySimplify = (double*)malloc(facilityCount*MAX_POINT_NUM*sizeof(double));
	int* facilitypixelCount = (int*)malloc(facilityCount*MAX_POINT_NUM*sizeof(int));
	bgi::rtree< valueP, bgi::quadratic<MAX_NODE_SIZE> > rtreeFacility;
	int facilityCountsimplify=0;
	...
	for(int i=0;i<facilityCount;i++)
	{
		...
		if(eType == wkbPoint)
		{
			...
			double pixelX=(int)((px-minXOut)/resolution);
			double pixelY=(int)((py-minYOut)/resolution);
			box facilitypBuffer;
			bg::buffer(bg::return_envelope<box>(point(pixelX,pixelY)),facilitypBuffer,0.1);
			std::vector<valueP> facilityResult;
			
			rtreeFacility.query(bgi::intersects(facilitypBuffer), std::back_inserter(facilityResult));
			if(facilityResult.size()<1)
			{
				rtreeFacility.insert(std::make_pair(point(pixelX,pixelY),facilityCountsimplify));
				facilitypxSimplify[facilityCountsimplify]=px;
				facilitypySimplify[facilityCountsimplify]=py;
				facilitypixelCount[facilityCountsimplify]=1;
				facilityCountsimplify++;
			}else{
				int facilityIndex=facilityResult.front().second;
				facilitypxSimplify[facilityIndex]=px + facilitypxSimplify[facilityIndex]*facilitypixelCount[facilityIndex];
				facilitypySimplify[facilityIndex]=py + facilitypySimplify[facilityIndex]*facilitypixelCount[facilityIndex];
				facilitypixelCount[facilityIndex]++;
				facilitypxSimplify[facilityIndex]/=facilitypixelCount[facilityIndex];
				facilitypySimplify[facilityIndex]/=facilitypixelCount[facilityIndex];
			}
		...
		else if( eType ==wkbPolygon)
		{
			...
			facilitypxSimplify[facilityCountsimplify]=px0;
			facilitypySimplify[facilityCountsimplify]=py0;
			facilityCountsimplify++;
	...
	//~ double tf= MPI_Wtime();
	//~ printf("[facility Simplify] Successfully. cores:%d coreid:%d cost:<total>%f</total> s\n",numProcs,myId, tf-t2);

	if(myId==0)
	printf("facilityCount:%d facilityCountsimplify:%d\n",facilityCount,facilityCountsimplify);
	for(int graphIndex=0;graphIndex<graphCount;graphIndex++)
	{
		...
		for(int i=myId;i<facilityCountsimplify;i+=numProcs)
		{

			double px = facilitypxSimplify[i];
			double py = facilitypySimplify[i];
	...
	//~ double t3= MPI_Wtime();
	//~ printf("[facility distance] Successfully. cores:%d coreid:%d cost:<total>%f</total> s\n",numProcs,myId, t3-t2);
